...
func commentChildList(c *gin.Context) {
	...
	//查询结果并返回
	_comment_child := []model.CommentQuestionChild{}
	mc.Table(model.CommentQuestionChild{}.Table()).Where(bson.M{"rid": _f.RID}).Limit(10).Find(&_comment_child)
	comment_child := []gin.H{}
	for _, v := range _comment_child {
		...
		if v.PID.Hex() != mc.Empty {
			co_child := model.CommentQuestionChild{}
